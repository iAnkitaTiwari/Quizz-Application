...
@login_required(login_url='adminlogin')
def admin_add_complexity(request):
    complexityForm=forms.ComplexityForm()
    if request.method=='POST':
        complexityForm=forms.ComplexityForm(request.POST)
        if complexityForm.is_valid():
            complexityForm.save()
        else:
            print("Form is invalid")
        return HttpResponseRedirect('/admin-view-complexity')
    return render(request, 'quiz/admin_add_complexity.html',{'complexityForm':complexityForm})            

@login_required(login_url='adminlogin')
def admin_view_add_complexity(request):
    return render(request, 'quiz/admin_view_add_complexity.html')


@login_required(login_url='adminlogin') 
def admin_view_complexity(request):
    complaxity= QMODEL.Complexity.objects.all()
    return render(request,'quiz/admin_view_complexity.html',{'complaxity':complaxity})
